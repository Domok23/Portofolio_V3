...
const socialLinks = [
  ...
  },
  {
    ...
    icon: ({ className, ...props }) => (
      <svg width="24px" height="24px" viewBox="0 0 45 45" version="1.1" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
        <title>Tiktok</title>
...
const LinkCard = ({ link, large = false }) => (
  <a
    ...
    className={`group relative flex items-center ${large ? 'justify-between p-4' : 'gap-3 p-4'} rounded-xl
  ...
  >
    ...
    <div className="relative flex items-center gap-4">
      <div className="relative flex items-center justify-center">
        ...
        <div className="relative p-2 rounded-md">
          <link.icon
            className={`${large ? 'w-6 h-6' : 'w-5 h-5'} transition-all duration-500 group-hover:scale-105`}
      ...
      <div className="flex flex-col min-w-0">
        <span className={`${large ? 'text-lg' : 'text-sm'} font-bold text-gray-200 tracking-tight leading-none group-hover:text-white transition-colors duration-300`}>
          {link.displayName}
        </span>
        <span className={`${large ? 'text-sm' : 'text-xs'} text-gray-400 truncate group-hover:text-gray-300 transition-colors duration-300`}>
          {link.subText}
        </span>
      </div>
    </div>
    ...
    <ExternalLink
      className={`${large ? 'w-5 h-5' : 'w-4 h-4 ml-auto'} text-gray-500 group-hover:text-white
